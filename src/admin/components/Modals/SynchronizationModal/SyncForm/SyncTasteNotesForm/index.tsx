...
interface ISyncTasteNotesForm extends Pick<ISyncFormType, 'control' | 'setValue' | 'values'> {}

export const SyncTasteNotesForm: FC<ISyncTasteNotesForm> = memo(({
  control,
  setValue,
  values,
}) => {
  ...
  const initialTasteNotes: ITasteNoteWithName[] = values?.sync_taste_notes;
  ...
  useEffect(() => {
    if (initialTasteNotes && initialTasteNotes.length > 0) {
      ...
      append([{ name: '', percent: null, id: null }, { name: '', percent: null, id: null }]); // Добавляем сразу два объекта
    }
  }, [append]);
  ...
  return (
    <FormSectionWrapper childrenClassName={styles.container}>
      {fields?.map((field, index) => (
        <div key={field.id} className={styles.wrapper}>
          <Controller
            name={`sync_taste_notes.${index}.id`}
            control={control}
            render={({ field: idField }) => <input type="hidden" {...idField} />}
          />

          <Controller
            name={`sync_taste_notes.${index}.name`}
            control={control}
            rules={{
              validate: (value) => {
            ...
            render={({ field: selectField, fieldState: { error } }) => (
              <SelectInput
                ...
                onChange={(item) => {
                  ...
                  if (setValue) {
                    setValue(`sync_taste_notes.${index}.id`, item.id);
                  }
                }}
                error={!!error}
          ...
          <Controller
            name={`sync_taste_notes.${index}.percent`}
            control={control}
            rules={{
              validate: (value) => {
...
SyncTasteNotesForm.displayName = 'SyncTasteNotesForm';
